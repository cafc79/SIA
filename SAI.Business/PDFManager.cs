...
namespace DS.SAI.Business
{
	public class PDFManager
	{
		...
		{
			if (File.Exists(ServerPath + "/logoDS.png"))
			{
				Image logo = Image.GetInstance(ServerPath + "/logoDS.png");
		...
		private PdfPTable ListaPartes()
		{
			...
			table.SetWidths(widths);
			table.Rows.Add(RowContent(new string[] {"No Parte DS", "No. Parte CLIENTE", "Descripción", "Afectación"}, 1));
			for (int i = 0; i < lPartList.Count; i++)
			{
				table.Rows.Add(i%2 > 0
				               	? RowContent(
				               		new string[]
				               			{
				               				lPartList[i].sPartListDS, lPartList[i].sPartListClient, lPartList[i].sPartListDescription,
