...
public partial class ExchangeNote : System.Web.UI.Page
{
	...
	protected void menuSolicitud_MenuItemClick(object sender, MenuEventArgs e)
	{
		...
		BlockControls();
		MenuSelected(util.GetActionMenu(mpMenu.SelectedItem.Value, false), iIdExchange);
	}
	...
	protected void imbSelectExchange_Click(object sender, ImageClickEventArgs e)
	{
		...
		selectedExchange(iIdExchange);
		wndRequerimiento.NavigateUrl = "Mobile/Requerimiento.aspx?ex=" + iIdExchange.ToString() + "&mod=ExchangeNote";
	...
	private void ControlState(bool state)
	{
		...
		for (int i = 0; i <= pnExchange.Controls.Count - 1; i++)
		{
            if (pnExchange.Controls[i].GetType().Name == "RadTextBox")
                ((RadTextBox)pnExchange.Controls[i]).Enabled = state;
		}
        for (int i = 0; i <= pnDocuments.Controls.Count - 1; i++)
		{
            if (pnDocuments.Controls[i].GetType().Name == "CheckBox")
                ((CheckBox)pnDocuments.Controls[i]).Enabled = state;
		}
	}
	...
	private void Requerimiento()
	{
		try
		{
			util.GetValidStage("imAddReq", mpMenu);
