...
public partial class Documentos : System.Web.UI.Page
{
    ...
    {
        var ex = new DS.SAI.Business.ExchangeManager();
        gvExchange.DataSource = ex.GetExchangePDF(txtOpPreFolio.Text, txtOpFolio.Text,
                                                  cbActivate.Checked ? ddlClient.SelectedValue : string.Empty,
                                                  dtOpR1.SelectedDate, dtOpR1.SelectedDate);
        gvExchange.DataBind();
    }
    ...
    protected void gridReporte_ItemCommand(object source, GridCommandEventArgs e)
    {
        if (e.CommandName == "Download")
        {
            try
            {
                ...
                if (util.DownloadRepOp(int.Parse(gridReporte.Items[item.ItemIndex]["Exchange"].Text),
                                       gridReporte.Items[item.ItemIndex]["Ruta"].Text, ref ruta))
