...
namespace BOCAR.SACI.Data
{
	public class PartListExchange : IPartListExchange
	{
			...
			try
			{
				...
				lst.AddRange(from DataRow dr in dt.Rows
				             select new PartListExchangeCompositeType
				                    	{
				                    		iIdPartListExchange = int.Parse(dr.ItemArray[0].ToString()), iIdPartList = int.Parse(dr.ItemArray[1].ToString()), iIdAffectation = int.Parse(dr.ItemArray[2].ToString()), iIdExchange = int.Parse(dr.ItemArray[3].ToString()), sPartListBocar = dr.ItemArray[4].ToString(), sPartListDescription = dr.ItemArray[5].ToString(), sPartListClient = dr.ItemArray[6].ToString(), sAffectationDescription = dr.ItemArray[7].ToString(), sFolio = dr.ItemArray[8].ToString(), sPreFolio = dr.ItemArray[9].ToString()
				                    	});
				return lst;
			}
			catch (Exception ex)
			...
			var et = new PartListExchangeCompositeType
			                                   	{
			                                   		...
			                                   		sPartListBocar = dt.Rows[0].ItemArray[4].ToString(),
		...
		//    et.sPartListBocar = dt.Rows[0].ItemArray[4].ToString();
