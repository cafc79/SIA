...
public partial class Reportes : System.Web.UI.Page
{
		...
		try
		{
			if (!Page.IsPostBack)
				util.LoadMenu(Master.Page.Request.Url.Segments[Master.Page.Request.Url.Segments.Length - 1],
				              BOCAR.SACI.Business.SecurityManager.ObtenerUsuario(HttpContext.Current).IdRol, ref menuReporte);
	...
	protected void menuSolicitud_MenuItemClick(object sender, MenuEventArgs e)
	{
		...
		String menu = util.GetActionMenu(menuReporte.SelectedItem.Value, false);
	...
	private void MenuSelected(string sAction)
	{
		try
		{
			util.GetValidStage(sAction, menuReporte);
			switch (sAction)
			{
				case "repOperArea":
