...
public partial class Catalogos : System.Web.UI.Page
{
		...
		try
		{
			if (!Page.IsPostBack)
				util.LoadMenu(Master.Page.Request.Url.Segments[Master.Page.Request.Url.Segments.Length - 1],
				              BOCAR.SACI.Business.SecurityManager.ObtenerUsuario(HttpContext.Current).IdRol, ref menuCatalog);
	...
	{
		cleanEv();
		switch (util.GetActionMenu(menuCatalog.SelectedItem.Value, false))
		{
	...
	{
		lblNumberPartList.Text = "No. Parte Bocar:";
	...
	protected void gridDescripcion_ItemCommand(object sender, GridCommandEventArgs e)
	{
		try
		{
			...
			{
				util.GetActionMenu(menuCatalog.SelectedItem.Value);
	...
	protected void gridTriara_DataBinding(object sender, EventArgs e)
	{
		...
		{
			case "sqlDSPartList":
				gridTriara.Columns[3].HeaderText = "No. Parte Bocar";
