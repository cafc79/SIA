...
public partial class Cotizacion : System.Web.UI.Page
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		{
			if (!Page.IsPostBack)
				util.LoadMenu(UrlReference, BOCAR.SACI.Business.SecurityManager.ObtenerUsuario(HttpContext.Current).IdRol,
				              ref menuSolicitud);
	...
	private void menuSelected()
	{
		...
		if ((util.GetActionMenu(menuSolicitud.SelectedItem.Value, false) != "imTiming") && (util.GetActionMenu(menuSolicitud.SelectedItem.Value, false) != "imQuote"))
			Response.Redirect(UrlReference + "?ex=" + Request.QueryString["ex"] + "&mod=" + util.GetActionMenu(menuSolicitud.SelectedItem.Value, false));
		else if (util.GetActionMenu(menuSolicitud.SelectedItem.Value, false) == "imTiming")
	...
	private BOCAR.SACI.Data.QuoteCompositeType HeaderReq()
	{
		try
		{
			var qct = new BOCAR.SACI.Data.QuoteCompositeType();
	...
	private BOCAR.SACI.Data.QuoteCompositeType calculate(BOCAR.SACI.Data.QuoteCompositeType qct)
	{
		decimal iSum = 0;
		decimal iSum2 = 0;
		if (chkMoldC.Checked)
	...
	private void Quote()
	{
		if (ddlPartListC.Items.Count > 0)
		{
			...
			var plct = new BOCAR.SACI.Data.PartListCompositeType();
			var plm = new PartListManager();

			var ect = new BOCAR.SACI.Data.ExchangeCompositeType();
