...
public partial class Timming : System.Web.UI.Page
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		try
		{
			if (!Page.IsPostBack)
			{
				...
				util.LoadMenu(ViewState["UrlReference"].ToString(), BOCAR.SACI.Business.SecurityManager.ObtenerUsuario(HttpContext.Current).IdRol,
				              ref menuSolicitud);
			}
	...
	{
		if (util.GetActionMenu(menuSolicitud.SelectedItem.Value, false) == "imQuote")
		...
		else if (util.GetActionMenu(menuSolicitud.SelectedItem.Value, false) != "imManAut")
			sqlDSExchangeTask.SelectCommand = "sp_selectExchangeTaskByIdExchange_Faltante";
		sqlDSExchangeTask.DataBind();
		if (util.GetActionMenu(menuSolicitud.SelectedItem.Value, false) != "imQuote")
			Response.Redirect(ViewState["UrlReference"].ToString() + "?ex=" + Request.QueryString["ex"] + "&mod=" + util.GetActionMenu(menuSolicitud.SelectedItem.Value, false));
	}
	#endregion
	...
	private void LoadAutorizacion()
	{
		...
		ddlAreaPendiente.DataSource = swap.AreaPendiente(int.Parse(Request.QueryString["ex"]), BOCAR.SACI.Business.SecurityManager.ObtenerUsuario(HttpContext.Current).IdEmpleado);
